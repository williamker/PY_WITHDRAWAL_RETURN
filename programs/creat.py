...
log_file = os.path.join(log_dir, "creat.log")
...
# ----------------- HELPERS -----------------
def setup_logging():
    logging.info("Démarrage du script creat.py (PY_WITHDRAWAL_RETURN / MAMT001).")
...
def extract_date14_from_fsts(fsts_line: str) -> str:
    ...
    Exemple vu: ...20260204202211BATCH...
    """
...
def normalize_extid(value: str, detail_prefix: str) -> str:
    """
    MAMT001: retirer le préfixe 9 chars "020000001" si présent,
    puis prendre le plus grand groupe numérique.
    """
    v = (value or "").strip()
    prefix9 = detail_prefix + "0000001"  # "020000001"
...
def process_one_aller(aller_file: str, retour_by_date: dict[str, str]) -> None:
    ...
    if not retour_file:
        raise ValueError(f"Aucun fichier retour MAMT001 correspondant à DATE={date14} dans {retour_dir}")
    ...
    logging.info(f"[MATCH] aller={os.path.basename(aller_file)} DATE_FSTS={date14} -> retour={os.path.basename(retour_file)} (DATE_RET={DATE}, BATCH_RET={BATCH})")
    ...
    # MAMT001 : statut en position 3122 => détail = 3121 chars avant append
    DETAIL_LEN = 3121
    DETAIL_PREFIX = "02"
    STATUS_BYTE_INDEX = 3121  # index 0-based en bytes où commence ACCT/RJCT
...
def main():
    ...
    if len(sys.argv) < 2:
        logging.error("Argument manquant: au moins un fichier INFINITE.MAMT001*")
        raise SystemExit(2)

    aller_files = sys.argv[1:]
    retour_files = list_retour_files_mamt001()
