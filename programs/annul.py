...
log_file = os.path.join(log_dir, "annul.log")
...
# ----------------- HELPERS -----------------
def setup_logging():
    logging.info("Démarrage du script annul.py (PY_WITHDRAWAL_RETURN / MAMT003).")
...
def normalize_extid(value: str, detail_prefix: str) -> str:
    """
    MAMT003 (ANNUL) : détails commencent par 04 et le RUI peut commencer par:
      "040000001..."
    On enlève les 9 premiers caractères si ça matche.
    """
    v = (value or "").strip()

    prefix9 = detail_prefix + "0000001"  # "040000001"
...
def list_retour_files_mamt003() -> list[str]:
    """
    Liste tous les retours MAMT003 selon ENV/patterns.
    """
    if ENV.lower() == "prod":
        pat = patterns.get("mamt003_retour_prod", "industryPROD.MAMT003_annul-mandats_*_R.csv")
    else:
        pat = patterns.get("mamt003_retour_nonprod", "grpindustry.industry-MAMT003_annul-mandats-*_R.csv")
...
def process_one_aller(aller_file: str, retour_by_date: dict[str, str]) -> None:
    ...
    # ANNUL : statut en position 228 => détail = 227 chars avant append
    DETAIL_LEN = 227
    DETAIL_PREFIX = "04"
    STATUS_BYTE_INDEX = 227  # index 0-based bytes où commence ACCT/RJCT
    ...
    with open(output_file, "wb") as out:
        ...
        for line_no, line in enumerate(aller_lines[1:], start=2):
            ...
            # sécurisation extraction RUI
            if len(raw) < 44:
                logging.warning(f"Ligne aller {line_no} trop courte ({len(raw)}), écrite brute.")
                out.write((raw + NEWLINE).encode(OUT_ENCODING, errors="strict"))
                nb_written += 1
                continue
            ...
            if len(raw) >= 79:
                ...
                logging.debug(f"Ligne aller {line_no}: longueur {len(raw)} < 79, RUM non injectée.")
...
def main():
    ...
    if len(sys.argv) < 2:
        logging.error("Argument manquant: au moins un fichier INFINITE.MAMT003*")
        raise SystemExit(2)

    aller_files = sys.argv[1:]
    retour_files = list_retour_files_mamt003()
