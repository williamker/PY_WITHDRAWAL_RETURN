...
log_file = os.path.join(log_dir, "modif.log")
...
# ----------------- HELPERS -----------------
def setup_logging():
    logging.info("Démarrage du script modif.py (PY_WITHDRAWAL_RETURN / MAMT002).")
...
def extract_date14_from_fsts(fsts_line: str) -> str:
    ...
    Exemple vu: ...20260205202036BATCH...
    """
...
def parse_partnerTsi_header_first_line(retour_file: str) -> tuple[str, str]:
    ...
    -> BATCH = parts[2] (chez toi souvent "0")
    """
...
def build_partnerTsi_map(retour_file: str) -> dict[str, tuple[str, str, str]]:
    """
    EXTID -> (statut_AC112, code_resultat_4, rum_35)
    """
...
def normalize_extid(value: str, detail_prefix: str) -> str:
    """
    MAMT002: retirer le préfixe 9 chars "030000001" si présent,
    puis prendre le plus grand groupe numérique.
    """
    v = (value or "").strip()
    prefix9 = detail_prefix + "0000001"  # "030000001"
...
def list_retour_files_mamt002() -> list[str]:
    if ENV.lower() == "prod":
        pat = patterns.get("mamt002_retour_prod", "industryPROD.MAMT002_modif-mandats_*_R.csv")
    else:
        pat = patterns.get("mamt002_retour_nonprod", "grpindustry.industry-MAMT002_modif-mandats-*_R.csv")
...
def process_one_aller(aller_file: str, retour_by_date: dict[str, str]) -> None:
    ...
    if not retour_file:
        raise ValueError(f"Aucun fichier retour MAMT002 correspondant à DATE={date14} dans {retour_dir}")
    ...
    # MAMT002 : statut en position 3000 => détail = 2999 chars avant append
    DETAIL_LEN = 2999
    DETAIL_PREFIX = "03"
    STATUS_BYTE_INDEX = 2999  # index 0-based en bytes où commence ACCT/RJCT
...
def main():
    ...
    if len(sys.argv) < 2:
        logging.error("Argument manquant: au moins un fichier INFINITE.MAMT002*")
        raise SystemExit(2)

    aller_files = sys.argv[1:]
    retour_files = list_retour_files_mamt002()
